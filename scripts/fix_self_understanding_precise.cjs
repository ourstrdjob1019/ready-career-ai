...
const newDefaultTestsStr = `      const defaultTests: DiagnosticTest[] = [
        {
          id: "test-riasec",
...
const newHandlersStr = `  const handleTakeTest = (test: DiagnosticTest) => {
...
// Delete lines 56 to 195 (indices 55 to 194) => length is 194 - 55 + 1 = 140
lines.splice(55, 140, newDefaultTestsStr);

// Since we changed the array size, line numbers below 195 are shifted!
// The original array length was 140 lines.
// The new string is 1 element in the array (with newlines inside).
// So we removed 140 elements, added 1 element. The shift is -139.
// Original lines 206 to 223 (indices 205 to 222) are now shifted by -139.
// New indices: 205 - 139 = 66, 222 - 139 = 83.
// Wait, to avoid index shifting math, I can just do it from bottom to top!
// Let's reload lines.
const newLines = content.split('\n');
// Bottom first: delete lines 206 to 223 (indices 205 to 222), length = 18
newLines.splice(205, 18, newHandlersStr);
// Top next: delete lines 56 to 195 (indices 55 to 194), length = 140
newLines.splice(55, 140, newDefaultTestsStr);

fs.writeFileSync(targetPath, newLines.join('\n'));
console.log('Fixed SelfUnderstanding.tsx precisely!');
