...
export const SelfUnderstanding: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-12 py-8 space-y-12 selection:bg-[#7B5CF0]/20 selection:text-[#7B5CF0] relative">
      ...
      {/* Toast */}
      {toastMsg && (
        <div className="fixed bottom-10 right-10 z-50 bg-[#7B5CF0] text-white px-6 py-4 rounded-3xl font-semibold tracking-tighter text-sm sm:text-base shadow-sm flex items-center gap-3 animate-bounce-short border-2 border-white">
      ...
      {/* =========================================================================
         ...
         ========================================================================= */}
      <div className="rounded-[36px] bg-gradient-to-r from-[#2B0E68] via-[#5A24CA] to-[#008A90] text-white p-8 sm:p-12 shadow-sm border-4 border-white/30 flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden print:hidden">
        <div className="space-y-4 max-w-2xl z-10 text-center sm:text-left">
          <span className="text-xs font-semibold tracking-tighter bg-[#FF3B7C] text-white px-4 py-1.5 rounded-full inline-flex items-center gap-1.5 shadow-md">
            <Brain className="w-4 h-4 text-amber-200" />
            <span>자기이해 진단 검사</span>
          </span>
          <h1 className="text-3xl sm:text-5xl font-headline font-semibold tracking-tighter tracking-tight leading-tight text-white">
            🧪 자기이해 진단 검사 7종
          </h1>
          ...
          <div className="pt-2 flex flex-wrap items-center gap-3 justify-center sm:justify-start">
            <span className="text-xs font-semibold tracking-tighter px-4 py-2 rounded-3xl bg-white/20 backdrop-blur-md border border-white/40">
              📊 내 진행 상태: <strong>6개 중 {completedCount}개 완료!</strong>
            </span>
            {completedCount === 6 && (
              <span className="text-xs font-semibold tracking-tighter px-4 py-2 rounded-3xl bg-gradient-to-r from-amber-400 to-amber-500 text-[#1A1626] shadow-lg animate-pulse">
                부석 6종 전부 완료!
              </span>
            )}
          </div>
      ...
      <div className="space-y-6 print:hidden">
        <div className="border-b-2 border-purple-150 pb-4 pl-2">
          <h2 className="text-2xl sm:text-3xl font-headline font-semibold tracking-tighter text-[#1A1626] flex items-center gap-2.5">
            <Sparkles className="w-7 h-7 text-[#7B5CF0]" />
            <span>📋 나의 자기이해 진단 목록</span>
          </h2>
          <p className="text-xs sm:text-sm font-semibold tracking-tighter text-[#5C5672] mt-1">
            ...
            return (
              <div
                ...
                className={`rounded-[36px] p-8 transition-all duration-300 flex flex-col justify-between space-y-6 border-3 relative ${
                  isDone
                    ? "bg-gradient-to-b from-[#F2FEFF] via-[#FAF6FF] to-white border-[#008A90]/60 shadow-sm hover:shadow-sm"
                    : "bg-white border-[#E4DCFF] shadow-sm hover:border-[#7B5CF0] hover:shadow-xl"
                }`}
              >
                {/* 상단 뱃지 및 상태 구별 */}
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold tracking-tighter bg-purple-100 text-[#6240D5] px-3 py-1 rounded-full">
                      {test.category}
                    </span>
                    {isDone ? (
                      <span className="text-xs font-semibold tracking-tighter bg-[#008A90] text-white px-3.5 py-1 rounded-full flex items-center gap-1 shadow-sm animate-pulse">
                        <CheckCircle2 className="w-3.5 h-3.5" />
                        <span>진단 완료!</span>
                      </span>
                    ) : (
                      <span className="text-xs font-semibold tracking-tighter bg-slate-100 text-slate-600 px-3.5 py-1 rounded-full">
                        ⚡ 미완료 ({test.timeEst})
                      </span>
                    )}
                  </div>

                  <h3 className="text-xl sm:text-2xl font-semibold tracking-tighter text-[#1A1626] leading-tight tracking-tight">
                    {test.title}
                  </h3>
                ...
                {/* 완료되었을 경우 요약 결과 미리보기 */}
                {isDone && (
                  <div className="bg-white p-4.5 rounded-3xl border-2 border-cyan-150 shadow-inner space-y-2">
                    <span className="text-[11px] font-semibold tracking-tighter text-[#008A90] flex items-center gap-1">
                      👑 진단 결과 요약:
                    </span>
                    <p className="text-xs font-semibold tracking-tighter text-[#1A1626]">
                      {test.resultType}
                    </p>
                    {test.scoreSummary && (
                      <span className="text-[11px] font-semibold tracking-tighter text-[#6E6A80] block bg-white p-1.5 rounded-lg border">
                        📈 {test.scoreSummary}
                      </span>
                    )}
                  </div>
                  ...
                  {isDone ? (
                    <>
                      <button
                        ...
                        className="w-full py-3.5 px-6 rounded-3xl bg-gradient-to-r from-[#008A90] to-[#00A3A8] hover:brightness-110 text-white font-semibold tracking-tighter text-sm shadow-lg flex items-center justify-center gap-2 cursor-pointer transform hover:scale-102 transition-all"
                      >
                      ...
                      <button
                        ...
                        className="w-full py-3 px-6 rounded-3xl bg-[#FAF7FF] hover:bg-[#F2EEFF] text-[#7B5CF0] border-2 border-purple-200 font-semibold tracking-tighter text-xs sm:text-sm transition-colors flex items-center justify-center gap-2 cursor-pointer"
                      >
                    ...
                    <button
                      ...
                      className="w-full py-4 px-6 rounded-3xl bg-gradient-to-r from-[#7B5CF0] to-[#6240D5] hover:brightness-110 text-white font-semibold tracking-tighter text-base shadow-xl flex items-center justify-center gap-2 cursor-pointer transform hover:scale-105 transition-all"
                    >
      ...
      {activeReportTest && (
        <div className="fixed inset-0 z-50 bg-black/65 backdrop-blur-md flex items-center justify-center p-4 sm:p-6 animate-fadeIn">
          <div className="bg-white w-full max-w-4xl rounded-[44px] p-8 sm:p-14 shadow-sm border-4 border-purple-200 relative max-h-[92vh] overflow-y-auto space-y-9 selection:bg-[#008A90]/20 modal-print-area">
            
            {/* 상단 모달 닫기 */}
            <button
              onClick={() => setActiveReportTest(null)}
              className="absolute top-7 right-8 p-3 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium tracking-tight shadow-sm transition-all"
            >
              <X className="w-6 h-6" />
            </button>
            ...
            <div className="border-b-4 border-[#7B5CF0] pb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
              <div className="space-y-2">
                <div className="inline-flex items-center gap-2 bg-[#008A90] text-white px-4 py-1 rounded-full text-xs font-semibold tracking-tighter shadow">
                  <Award className="w-4 h-4 text-amber-300" />
                  <span>2026 ReadyCareer AI Official Diagnostic Report</span>
                </div>
                <h2 className="text-2xl sm:text-4xl font-headline font-semibold tracking-tighter text-[#1A1626]">
                  📑 {activeReportTest.title} 결과 리포트
                </h2>
                <p className="text-xs sm:text-sm font-semibold tracking-tighter text-[#5C5672]">
              ...
              <button
                ...
                className="py-4 px-8 rounded-3xl bg-gradient-to-r from-[#FF3B7C] to-[#FF7043] hover:brightness-110 text-white font-semibold tracking-tighter text-sm sm:text-base shadow-xl flex items-center justify-center gap-3 transform hover:scale-105 transition-all cursor-pointer flex-shrink-0"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="md:col-span-1 bg-[#FAF7FF] p-6 rounded-[32px] border-2 border-purple-200 shadow-inner flex flex-col justify-between space-y-4 text-center">
                <div className="space-y-2">
                  <span className="text-xs font-semibold tracking-tighter bg-[#7B5CF0] text-white px-3 py-1 rounded-full inline-block">
                    대표 판정 결과
                  </span>
                  <h4 className="text-xl font-semibold tracking-tighter text-[#1A1626] pt-2 leading-snug">
                    {activeReportTest.resultType || "AI 고위험 융합 주도형"}
                  </h4>
                </div>
                <div className="w-24 h-24 rounded-full bg-gradient-to-tr from-[#7B5CF0] to-[#008A90] p-1 shadow-lg mx-auto flex items-center justify-center text-white font-semibold tracking-tighter text-2xl">
                  A+
                </div>
                <span className="text-xs font-semibold tracking-tighter text-[#008A90] bg-white py-2 rounded-3xl border border-cyan-200 block">
                  ✨ 상위 3% 이내 우수지표
                </span>
              </div>

              <div className="md:col-span-2 bg-[#F8FAFF] p-7 rounded-[32px] border-2 border-gray-200 shadow-sm space-y-4 flex flex-col justify-center">
                <span className="text-xs font-semibold tracking-tighter text-black flex items-center gap-1.5">
                  <Brain className="w-4 h-4" />
                  <span>AI 사정관의 종합 심층 해부 요약</span>
                </span>
                <p className="text-base sm:text-lg font-semibold tracking-tighter text-[#1A1626] leading-relaxed">
                  "{activeReportTest.reportDetails.summary}"
                </p>
                <div className="pt-2">
                  <span className="text-xs font-medium tracking-tight text-[#6E6A80]">
            ...
            <div className="bg-white p-7 sm:p-8 rounded-[32px] border-2 border-purple-100 shadow-sm space-y-6">
              <h4 className="text-xl font-semibold tracking-tighter text-[#1A1626] flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {activeReportTest.reportDetails.scores.map((sc, idx) => (
                  <div key={idx} className="bg-[#FAF8FD] p-5 rounded-3xl border border-purple-200/60 space-y-2">
                    <div className="flex items-center justify-between text-sm font-semibold tracking-tighter text-[#1A1626]">
                      <span>{sc.label}</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="bg-[#FAF7FF] p-7 rounded-[32px] border-2 border-[#DED4FF] space-y-4">
                <span className="text-sm font-semibold tracking-tighter text-[#6240D5] flex items-center gap-2">
                  <Target className="w-5 h-5" />
                  <span>🎯 진정성 100% 실천 가이드 &amp; 과제 추천</span>
                </span>
                <ul className="space-y-3 pt-1 text-xs sm:text-sm font-semibold tracking-tighter text-[#3F3952]">
              ...
              <div className="bg-gradient-to-br from-[#E6FAFE] to-[#FAF3FF] p-7 rounded-[32px] border-2 border-[#BFF6FE] space-y-3 flex flex-col justify-between">
                <div className="space-y-2">
                  <span className="text-xs font-semibold tracking-tighter bg-[#008A90] text-white px-3 py-1 rounded-full inline-block">
                    ✨ 희망 직무("{targetJobName}") 시너지 효과
                  </span>
                  <p className="text-sm sm:text-base font-semibold tracking-tighter text-[#1A1626] leading-relaxed pt-1">
                    {activeReportTest.reportDetails.aiCareerComment}
                  </p>
                </div>
                <span className="text-[11px] font-semibold tracking-tighter text-[#006970] text-right">
            ...
            <div className="pt-4 flex flex-col sm:flex-row items-center justify-between gap-4 border-t-2 border-purple-100">
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="py-3.5 px-6 rounded-3xl bg-slate-100 hover:bg-slate-200 text-slate-800 font-semibold tracking-tighter text-xs sm:text-sm"
                >
                  🔄 검사 다시 치르기 (Retake)
                </button>
                <button
                  onClick={() => setActiveReportTest(null)}
                  className="py-3.5 px-8 rounded-3xl bg-[#7B5CF0] hover:bg-[#6240D5] text-white font-semibold tracking-tighter text-xs sm:text-sm shadow-lg"
                >
                  확인 및 창 닫기 &rarr;
                </button>
              </div>
      ...
      {quickTestModal && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-md flex items-center justify-center p-4 animate-fadeIn">
          <div className="bg-white w-full max-w-2xl rounded-[40px] p-8 sm:p-12 shadow-2xl border-4 border-[#DED4FF] relative text-center space-y-8">
            ...
            <button onClick={() => setQuickTestModal(null)} className="absolute top-6 right-6 p-2 rounded-full bg-slate-100 font-medium tracking-tight">
              <X className="w-6 h-6" />
            </button>

            <div className="space-y-2 border-b border-purple-100 pb-5">
              <span className="text-xs font-semibold tracking-tighter bg-[#7B5CF0] text-white px-3.5 py-1 rounded-full inline-block">
                ⚡ 2026 실무 맞춤 쾌속 3제 진단 진행 중
              </span>
              <h3 className="text-2xl sm:text-3xl font-semibold tracking-tighter text-[#1A1626]">
                {quickTestModal.title}
              </h3>
              <p className="text-xs font-medium tracking-tight text-[#6E6A80]">
            ...
            <div className="bg-[#FAF7FF] p-7 rounded-[32px] border-2 border-purple-200 shadow-inner space-y-6 text-left">
              <h4 className="text-lg sm:text-xl font-semibold tracking-tighter text-[#1A1626] leading-relaxed">
              ...
              <div className="space-y-3.5">
                {[
                  ...
                  <button
                    ...
                    className="w-full p-4.5 rounded-3xl bg-white hover:bg-[#EAFEFE] border-2 border-purple-200 hover:border-[#008A90] text-left font-semibold tracking-tighter text-xs sm:text-sm text-[#3B364C] hover:text-[#006970] shadow-sm hover:shadow-md transition-all flex items-center justify-between group cursor-pointer"
                  >
                    <span>{opt}</span>
            ...
            <div className="text-xs font-semibold tracking-tighter text-[#8D88A0]">
